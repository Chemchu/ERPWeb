...
const FinanceCard = (props: {
  titulo: string;
  unidad?: string;
  dataActual: string | undefined;
  dataPrevio: string | undefined;
}) => {
      ...
      <div className="flex gap-2 justify-center items-center">
        <span className="rounded-lg relative p-4 bg-purple-200">
          <svg
            ...
            className="text-purple-500 h-6 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
        ...
        <p className="text-lg text-black dark:text-white">{props.titulo}</p>
      </div>
      <div className="flex flex-col items-center">
        <p className="text-gray-700 dark:text-gray-100 text-4xl text-left font-bold my-4">
          {props.dataActual}
          <span className="text-sm">{props.unidad || "€"}</span>
        </p>
      </div>
      {!isNaN(Number(crecimiento)) && Number(crecimiento) !== 0 && (
        <div
          className={`flex justify-center gap-1 items-center ${
            isCrecimientoPositivo ? "text-green-500" : "text-red-500"
          } text-sm`}
        >
          ...
          <span>{isCrecimientoPositivo ? crecimiento : "-" + crecimiento}%</span>
          <span className="text-gray-400 text-xs">vs ayer</span>
        </div>
      )}
    </div>
  );
