...
const VentasDelDia = (props: {
  ...
  const CustomTooltip = ({ active, payload, label }: TooltipProps<ValueType, NameType>) => {
    let horaInicial =
      String(label).length < 5 ? Number(String(label).substring(0, 1)) : Number(String(label).substring(0, 2));
    ...
    if (active) {
      return (
        <div className="bg-white border border-blue-600 opacity-90 rounded-xl shadow-lg p-4 custom-tooltip">
          ...
          {payload && <p className="label text-base">{`Ventas: ${Number(payload[0].value).toFixed(2)}€`}</p>}
        </div>
      );
    }
  ...
  return (
    <div className="w-full rounded-xl border-x border-b shadow-xl hover:shadow-2xl">
      <div className="flex justify-center py-4 font-semibold text-gray-600">{props.titulo}</div>
      <div className="w-full h-full ">
        <ResponsiveContainer width="100%" height="100%" minHeight={250}>
          <AChart data={ventas} margin={{ top: 0, right: 30, left: 0, bottom: 10 }}>
            <defs>
            ...
            {props.maxY ? <YAxis dataKey={props.ejeY} domain={[0, props.maxY]} /> : <YAxis dataKey={props.ejeY} />}
            <CartesianGrid strokeDasharray="3 3" />
            <Tooltip content={CustomTooltip} />
            <Area
              type="monotone"
              dataKey={props.ejeY}
              name={props.nombreEjeX}
