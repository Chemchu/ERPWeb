...
const Dropdown = (props: {
  ...
  return (
    <Combobox
      ...
      value={props.selectedElemento}
      onChange={(e: any) => props.setElemento(e)}
    >
      ...
      <Transition leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
        <Combobox.Options className="absolute flex flex-col py-1 mt-1 bg-white w-full border rounded-lg z-10 border-blue-500">
          {filteredOptions.slice(0, 10).map((elem, index) => (
            <Combobox.Option
              className="flex h-8 hover:bg-blue-400 hover:text-white cursor-pointer items-center border-b border-blue-500"
              key={`${elem}-${index}`}
              value={elem}
            >
              <span className="p-4">{elem}</span>
            </Combobox.Option>
          ))}
        </Combobox.Options>
      </Transition>
