...
const UploadFile = (props: { extension?: string[]; tipoDocumento: TipoDocumento }) => {
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (!f.type) {
        notifyError("El archivo no tiene tipo");
        return;
      }
      if (!Boolean(props.extension) && !TIPOS_PERMITIDOS.includes(`${f.type.substring(5)}`)) {
        notifyError(`El archivo no es del tipo ${f.type}`);
        return;
      }
  ...
  return (
    <div className="flex items-center">
      <label
        className="flex flex-shrink-0 justify-center gap-2 w-28 py-2 text-base font-semibold 
        cursor-pointer text-white bg-amber-500 rounded-lg shadow-md hover:bg-amber-600 focus:outline-none
         focus:ring-2 focus:ring-amber-600 focus:ring-offset-2 focus:ring-offset-blue-200"
      >
        <span>Importar</span>
        <svg
