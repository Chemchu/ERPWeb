...
const EditarVenta = (props: {
  ...
  const ModificarTipoVenta = async () => {
    const { message, successful } = await ModificarVenta(
      props.venta._id,
      tipoVenta
    );
    if (!successful) {
  ...
  return (
    ...
    >
      <Backdrop onClick={() => props.setShowModal(false)}>
        ...
        >
          <span className="text-left text-2xl w-full">Modificar venta</span>
          <div className="grow w-full h-full">
            <span>Tipo de venta</span>
            <SimpleListBox
              elementos={["Efectivo", "Tarjeta"]}
              setElemento={setTipoVenta}
            />
          </div>
          ...
          <AnimatePresence>
            {showModal && (
              <ConfirmacionModal
                ...
                title="¿Modificar el tipo de venta?"
                subtitle={`La venta pasará de ${props.venta.tipo} a ${tipoVenta}`}
                acceptCallback={async () => {
                  await ModificarTipoVenta();
                }}
