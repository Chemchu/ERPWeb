...
export const CerrarCaja = (props: { Empleado?: SesionEmpleado }) => {
  ...
  useEffect(() => {
    ...
    const GetVentas = async (j: SesionEmpleado) => {
      if (!j.TPV) {
        notifyError("El empleado no está usando la TPV");
        return;
      }

      const tpv = await FetchTPV(j.TPV, abortController);
      if (!tpv) {
        notifyError("No se ha encontrado la TPV que se quiere cerrar");
        return;
      }
  ...
  const CerrarCaja = async () => {
    ...
    try {
      ...
      const cierre = await NuevoCierreTPV(
        Empleado,
        SetEmpleado,
        Number(TotalEfectivo),
        Number(TotalTarjeta),
        Number(DineroRetirado),
        Number(TotalPrevistoEnCaja),
        Number(TotalRealEnCaja),
        abortController
      );
  ...
  if (!Ventas) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full">
        ...
        >
          <motion.div
            className="h-3/6 w-2/6 m-auto py-2 flex flex-col gap-4 items-center justify-center bg-white rounded-2xl"
  ...
  if (anyadiendoCierre) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full ">
        ...
        >
          <motion.div
            className="h-1/3 w-1/3 flex flex-col items-center bg-white rounded-2xl p-4"
            onClick={(e) => e.stopPropagation()}
            variants={In}
            initial="hidden"
            animate="visible"
          ...
          >
            ...
            {Cierre && Tpv && (
              <div style={{ display: "none" }}>
                <CierrePrintable ref={componentRef} cierre={Cierre} tpv={Tpv.nombre} qrImage={qrImage} />
              </div>
            )}
          </motion.div>
        </Backdrop>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full ">
      ...
      >
        <motion.div
          className="h-4/6 w-4/6 flex flex-col items-center bg-white rounded-2xl p-4"
          onClick={(e) => e.stopPropagation()}
          variants={In}
          initial="hidden"
          animate="visible"
        ...
        >
          <div className="flex flex-col gap-4 h-full w-full rounded justify-center content-center">
            <div className="text-lg text-center font-bold">Ventana de cierre de TPV</div>
            <hr />
            <div className="flex w-full h-full">
              <div className="flex flex-col gap-2 xl:gap-4 w-full h-full">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <input
                    className="rounded-lg border p-1 outline-blue-500 text-right"
                    onChange={(e) => {
                      SetTotalReal(e.target.value);
                    }}
                    value={TotalRealEnCaja}
                  />
                  €
                  <div
                    className="flex hover:bg-blue-200 rounded-full cursor-pointer w-8 h-8 items-center justify-center"
                    onClick={() => {
                      setContarCaja((isOpen) => !isOpen);
                    }}
                  >
                    <svg
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <input
                    className="rounded-lg border p-1 outline-blue-500 text-right"
                    onChange={(e) => {
                      setDineroRetirado(ValidatePositiveFloatingNumber(e.target.value));
                    }}
                    value={DineroRetirado}
                  />
                  €
                </div>
              </div>
            ...
            <div className="flex gap-4 h-auto w-full text-white">
              <div
                className="flex h-10 w-2/3 m-auto bg-red-500 hover:bg-red-600 rounded-xl cursor-pointer items-center justify-center shadow-lg"
                onClick={() => {
                  CerrarCajaState.setShowCerrarCajaModal(false);
                }}
              >
              ...
              {Number(TotalRealEnCaja) > 0 &&
              Number(TotalRealEnCaja) - Number(DineroRetirado) >= 0 &&
              !isButtonDisabled ? (
                <div
                  className={`flex h-10 w-2/3 m-auto bg-blue-500 hover:bg-blue-600 rounded-xl cursor-pointer items-center justify-center shadow-lg`}
                  onClick={() => {
                    setConfimationRequest(true);
                  }}
                >
                  <div>Cerrar TPV</div>
                </div>
                ...
                <div
                  className={`flex h-10 w-2/3 m-auto bg-blue-400 rounded-xl cursor-default items-center justify-center shadow-lg`}
                >
                  <div>Cerrar TPV</div>
                </div>
              )}
          ...
          <AnimatePresence>
            {showContarCaja && (
              <ContarCaja
                showItself={setContarCaja}
                setTotal={setTotalRealEnCaja}
                desglose={Desglose}
            ...
            {confimationRequest && (
              <RequestConfirmacion
                titulo="¿Desea añadir el cierre?"
                msg="El cierre no se podrá modificar una vez sea añadido"
