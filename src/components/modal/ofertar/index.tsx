  ...
  if (isLoading) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full ">
        ...
        >
          <motion.div
            className="flex flex-col h-3/4 w-3/4 xl:h-1/2 xl:w-1/2 gap-2 items-center bg-white rounded-2xl p-4"
          ...
          >
            ...
            <div className="flex w-full items-end justify-around text-white gap-10">
              ...
              <button className="h-10 w-full rounded-xl bg-blue-500 hover:bg-blue-600 shadow-lg">Añadir oferta</button>
            </div>
          </motion.div>
        </Backdrop>
      </motion.div>
        ...
        >
          ...
          >
            ...
            <div className="flex gap-1 w-full text-xl items-center">
              ...
              <AnimatePresence>
                {infoOpen && (
                  <FloatingLabel texto="Esta compra no cumple con los requisitos de oferta" color="orange" />
                )}
              </AnimatePresence>
            </div>
            ...
            <div className="flex w-full items-end justify-around text-white gap-10">
              ...
              {ofertas.length <= 0 ? (
                ...
                <button
                  className="h-10 w-full rounded-xl bg-blue-500 hover:bg-blue-600 shadow-lg cursor-pointer"
                  onClick={() => {
                    AddProductToSale();
                  }}
                >
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col w-full justify-center">
            <ProductoPicker
              titulo="Escriba el nombre del producto a ofertar"
              productos={props.productos}
              familiaOfertada={familiaOfertada}
          ...
          <div className="w-full h-full rounded-lg border overflow-y-scroll">
            {ofertas.map((prod, index) => {
              return (
                <div className="w-full" key={`oferta-${prod.nombre}-${index}`} ref={messagesEndRef}>
...
const ProductoPicker = (props: {
  titulo: string;
  productos: Producto[];
  familiaOfertada: string;
  setOfertas: React.Dispatch<React.SetStateAction<Producto[]>>;
}) => {
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      <div className="flex gap-1 w-full h-full items-center">
        <Dropdown
          elementos={props.productos
            .filter((p) => p.familia === props.familiaOfertada)
            .map((p) => {
              return p.nombre;
            })}
