...
export const ModalPagar = (props: {
  ...
  const RealizarVenta = async (pagoCliente: CustomerPaymentInformation) => {
    ...
    try {
      ...
      UpdatePaymentInfo();
      if (!Empleado || !Empleado.TPV) {
        notifyError("Error con la autenticación");
        return;
      }
  ...
  const GetFormaDePago = (): string => {
    if (Number(dineroEntregado) > 0 && Number(dineroEntregadoTarjeta) > 0) {
      return TipoCobro.Fraccionado.toString();
    }
    if (Number(dineroEntregadoTarjeta) > 0) {
      return TipoCobro.Tarjeta.toString();
    }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        <motion.div
          className="flex w-11/12 max-h-full items-center bg-white rounded-xl"
          ...
          <div id="receipt-content" className="flex flex-col gap-4 text-left w-full text-sm p-4 overflow-auto">
              ...
              <div className="h-full w-full bg-white text-center justify-center">
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col justify-between text-lg text-center">
                    ...
                    <Dropdown
                      elementos={Clientes.map((c) => {
                        return c.nombre;
                      })}
                      selectedElemento={NombreClienteActual}
                      setElemento={SetNombreClienteActual}
                    />
                  </div>
                </div>
                <div className="flex flex-col gap-2 justify-items-start w-full">
                  <div className="text-left">
                    ...
                    <label className="text-base" htmlFor="NombreCompleto">
              ...
              <div className="flex flex-col w-full bg-white text-center justify-center">
                <div className="flex flex-col gap-4 items-center">
                  ...
                  <div className="flex xl:gap-10 gap-4">
                    <div>
                      <div className="text-xl text-left">Efectivo</div>
                      <div className="text-xl">
                        <InputNumber
                          value={dineroEntregado}
                          setValue={SetDineroClienteEfectivo}
                          tipo={TipoCobro.Efectivo}
                          valorPendiente={cambio.toFixed(2)}
                        />
                      </div>
                    </div>
                    <div>
                      <div className="text-xl text-left">Tarjeta</div>
                      <div className="text-xl">
                        <InputNumber
                          value={dineroEntregadoTarjeta}
                          setValue={SetDineroClienteTarjeta}
                          tipo={TipoCobro.Tarjeta}
                          valorPendiente={cambio.toFixed(2)}
                        />
                      </div>
                    </div>
                ...
                <div className="flex gap-10 text-lg text-center justify-center">
                  ...
                  <div>
                    ...
                    <div className="text-4xl font-semibold">
                      {isNaN(Number(dineroEntregado) + Number(dineroEntregadoTarjeta))
                        ? "0.00"
                        : (Number(dineroEntregado) + Number(dineroEntregadoTarjeta)).toFixed(2)}
                      €
                    </div>
                  </div>
                  <div>
                    <div>{Number(cambio.toFixed(2)) < 0 ? "Pendiente" : "Cambio"}</div>
                    <div className={`text-4xl font-semibold ${cambio < 0 ? "text-red-500" : "text-green-500"}`}>
                      {cambio.toFixed(2)}€
                    </div>
                  </div>
                </div>
            ...
            <div className="flex justify-items-center gap-2">
              <button
                ...
                className={`${
                  isButtonDisabled ? "bg-red-400 cursor-default" : "bg-red-500 hover:bg-red-600"
                } text-white w-full h-10 hover:shadow-lg rounded-lg flex items-center justify-center`}
              ...
              {serverUp ? (
                Number(cambio.toFixed(2)) >= 0 ? (
                  <button
                    disabled={isButtonDisabled}
                    className={`${
                      isButtonDisabled ? "bg-blue-400 cursor-default" : "bg-blue-500 hover:bg-blue-600"
                    } text-white w-full h-10 hover:shadow-lg rounded-lg flex items-center justify-center`}
                    onClick={async (e) => {
                      await RealizarVenta(PagoDelCliente);
                    }}
                  >
