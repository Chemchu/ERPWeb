...
const BorrarCierreModal = (props: { showModal: Function, showCierreModal: Function, setCierre: Function, cierre: Cierre }) => {

    const Eliminar = async (id: string) => {
        const deletedCorrectly = await DeleteCierre(id);

        if (deletedCorrectly) {
            props.showCierreModal(false);
                    ...
                    <div className="w-full h-full text-center">
                        <div className="flex h-full flex-col justify-between">
                            ...
                            <p className="text-gray-800 dark:text-gray-200 text-xl font-bold mt-4">
                                {`Borrar cierre de ${new Date(Number(props.cierre.cierre)).toLocaleString()}`}
                            </p>
                            <p className="text-gray-600 dark:text-gray-400 text-xs py-2 px-6">
                                ¿Seguro de que quieres borrar el cierre?
                            </p>
                            <div className="flex items-center justify-between gap-4 w-full mt-8">
                                <button onClick={async () => await Eliminar(props.cierre._id)} type="button" className="py-2 px-4  bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 focus:ring-offset-blue-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg ">
                                    Sí
                                </button>
...
export default AuthorizationWrapper([Roles.Administrador, Roles.Gerente])(BorrarCierreModal);
//export default BorrarCierreModal;
