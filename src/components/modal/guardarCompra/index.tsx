...
const GuardarCompra = (props: { setModal: Function; compraActual: ProductoVendido[]; setCompraActual: Function }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full ">
      ...
      >
        <motion.div
          className="flex flex-col h-1/3 w-1/3 items-center bg-white rounded-2xl p-4"
        ...
        >
          ...
          <div className="flex gap-4 w-full justify-around items-end text-white ">
            ...
            {nombreCliente ? (
              ...
              <button
                className="w-1/3 h-12 rounded-xl bg-blue-400 cursor-default shadow-lg"
                onClick={() => notifyWarn("No se puede guardar sin el nombre del cliente")}
              >
                Guardar
              </button>
            )}
