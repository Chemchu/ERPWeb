...
const StoreDataModal = (props: { showModal: Function }) => {
  ...
  const Aceptar = () => {
    ...
    Cookies.set("storeData", JSON.stringify({ nombre: nombreTienda, direccion: direccion, cif: cif }), {
      expires: 365 * 3,
    });
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        <motion.div
          ...
          exit="exit"
        >
          <span className="w-full text-3xl text-left">Datos de la tienda</span>
          <div className="flex gap-2 w-full h-full items-center">
            <label className="w-full block tracking-wide text-gray-700 font-bold">Nombre de la tienda</label>
            <input
          ...
          <div className="flex gap-2 w-full h-full items-center">
            <label className="w-full block tracking-wide text-gray-700 font-bold">CIF</label>
            <input
              className="appearance-none ring-blue-500 block w-full bg-gray-200 text-gray-700 border rounded p-2 leading-tight focus:outline-none focus:bg-white"
              type="text"
