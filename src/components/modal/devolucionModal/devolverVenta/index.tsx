...
const DevolverVenta = (props: { venta: Venta; setModal: Function; setModalVenta: Function }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        <motion.div
          className="h-2/3 w-4/6 flex flex-col gap-4 items-center justify-between bg-white rounded-2xl p-4"
          ...
          exit="exit"
        >
          <span className="text-xl w-full text-left font-semibold">Selecciona los productos a devolver</span>

          <ListaDevolucionProductos
            key={`listaKey::${props.venta.productos.length}`}
            listaProductos={props.venta.productos}
            productosDevolver={ProductosDevolver}
            setProductosDevolver={setProductosDevolver}
          />

          {props.venta.tipo === TipoCobro.Fraccionado && (
            <span className="italic">
              Recuerda: si la venta fue fraccionada, toda la devolución se hará en efectivo
            </span>
          )}
          ...
          <div className="flex gap-4 w-full h-1/6 justify-around items-end text-white">
            ...
            {ProductosDevolver.size > 0 ? (
              <button
                className={`w-full h-12 rounded-xl shadow-lg bg-blue-500 hover:bg-blue-600 cursor-pointer`}
                onClick={async () => {
                  await AceptarReembolso();
                }}
              >
                Aceptar
              </button>
            ) : (
              <button className={`w-full h-12 rounded-xl shadow-lg bg-blue-400 cursor-default`}>Aceptar</button>
            )}
          </div>
          {qrImage && Devolucion && (
