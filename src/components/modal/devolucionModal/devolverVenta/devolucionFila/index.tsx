...
const DevolucionFila = (props: {
  producto: ProductoVendido;
  productosMarcadosMap: Map<string, number>;
  setProductosMarcados: Function;
}) => {
  ...
  useEffect(() => {
    if (checked) {
      ...
      if (prodsMap.has(props.producto._id)) {
        return;
      }
      ...
      if (!props.productosMarcadosMap.has(props.producto._id)) {
        return;
      }
  ...
  const ValueInput = (e: string) => {
    ...
    if (Number(e) > props.producto.cantidadVendida) {
      return;
    }
  ...
  const AddCantidad = () => {
    setCantidad((cantidad) => {
      ...
      if (Number(cantidad) >= props.producto.cantidadVendida) {
        return cantidad;
      }
  ...
  return (
    <li className="flex gap-2 items-center justify-start border-2 rounded-md px-2 py-1 hover:border-blue-400 hover:bg-blue-100">
      <input
        type="checkbox"
        name={`checkbox::${props.producto._id}`}
        id={`idInput::${props.producto._id}`}
      ...
      <AnimatePresence>
        {checked && (
          ...
          >
            <button
              className={`${
                cantidad === "1" ? "bg-blue-300 cursor-default" : "bg-blue-500 hover:bg-blue-600"
              } rounded w-10 h-8 text-white font-bold`}
              onClick={SubCantidad}
            >
              -
            </button>
            <input
              type="text"
              className="w-12 xl:w-20 border-2 border-blue-400 focus:outline-blue-500 rounded-md text-center"
              value={cantidad}
              onBlur={OnFocusLost}
            ...
            <button
              className={`${
                Number(cantidad) === props.producto.cantidadVendida
