...
const ListaDevolucionProductos = (props: {
  listaProductos: ProductoVendido[];
  productosDevolver: Map<string, number>;
  setProductosDevolver: Function;
}) => {
  return (
        ...
        {props.listaProductos.map((p) => {
          return (
            <DevolucionFila
              key={`id::${p._id}`}
              producto={p}
              productosMarcadosMap={props.productosDevolver}
              setProductosMarcados={props.setProductosDevolver}
            />
          );
        })}
      </ul>
