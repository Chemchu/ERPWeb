  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <Backdrop onClick={() => AbrirCajaState.setShowAbrirCajaModal(false)}>
        ...
        >
          <div className="text-2xl justify-self-start p-4">Abrir TPV</div>

          <div className="flex flex-col flex-grow gap-10 justify-center">
            <div className="flex gap-10">
              <div className="self-center ">Selecciona la TPV</div>
              <div className="w-full self-center justify-end">
                <Droplist
                  selectedElemento={currentTpvName ? currentTpvName : "Cargando..."}
                  elementos={tpvs.map((a) => {
                    return a.nombre;
                  })}
                  setElemento={setCurrentTpvName}
                />
              </div>
            </div>

            <div className="flex gap-10">
              <div className="self-center">Caja inicial</div>
              <div className="flex gap-2 self-center ml-auto items-center">
                <input
                  ...
                  name="cajaInicial"
                  id="cajaInicial"
                  placeholder=" Por ejemplo 350.50"
                  ...
                  onChange={(e) => {
                    setCajaInicial(ValidatePositiveFloatingNumber(e.target.value));
                  }}
                  value={cajaInicial}
                />
                <span>€</span>
          ...
          <div className="flex w-full gap-4 text-center justify-end items-end text-white p-4">
            <div
              className="flex h-10 w-full m-auto bg-red-500 hover:bg-red-600 rounded-xl cursor-pointer items-center justify-center shadow-lg"
              onClick={() => {
                AbrirCajaState.setShowAbrirCajaModal(false);
              }}
            >
            ...
            {Number(cajaInicial) > 0 && currentTpvName && tpvs ? (
              <div
                className="flex h-10 w-full m-auto bg-blue-500 hover:bg-blue-600 rounded-xl cursor-pointer items-center justify-center shadow-lg"
                onClick={AbrirTPV}
              >
                <div>Abrir TPV</div>
              </div>
          ...
          <AnimatePresence>
            {showContador && (
              <ContarCaja
                showItself={setContador}
                desglose={desglose}
                setDesglose={setDesglose}
                setTotal={setCajaInicial}
              />
            )}
          </AnimatePresence>
        </motion.div>
