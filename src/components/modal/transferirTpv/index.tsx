...
const TransferirTpv = () => {
  ...
  const Transferir = async () => {
    if (!Empleado.TPV) {
      notifyError("No se puede transferir una TPV que no se está usando");
      return;
    }

    if (!empleadoSeleccionado) {
      notifyWarn("Seleccione el empleado al que le quiere transfirir el control de la TPV");
      return;
    }
    const regex = /(?<=\[)(.*?)(?=\])/;
  ...
  if (empleados.length <= 0 || isTransfiriendoTPV) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full ">
        ...
        >
          <motion.div
            className="flex h-2/5 w-2/5 items-center justify-center bg-white rounded-2xl p-4"
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full w-full ">
      ...
      >
        <motion.div
          className="flex flex-col h-2/5 w-2/5 items-center bg-white rounded-2xl p-4"
          onClick={(e) => e.stopPropagation()}
          variants={In}
          initial="hidden"
          animate="visible"
        ...
        >
          ...
          <section className="flex flex-col gap-4 h-full w-full items-center justify-center px-2">
            <div className="w-full h-auto">
              <SimpleListBox
                elementos={empleados.map((emp) => `${emp.nombre} ${emp.apellidos} - [${emp.email}]`)}
                setElemento={setEmpleadoSeleccionado}
              />
            </div>
            {empleadoSeleccionado && (
              <span className="text-sm italic w-full text-center">
                Se pasará el control de la TPV a {empleadoSeleccionado.split("-")[0]}
              </span>
            )}
            <div className="flex items-end gap-2 w-full h-full text-white">
              <button
                onClick={() => TransferirTPVState.setShowTransferirTPVModal(false)}
                className="bg-red-500 hover:bg-red-600 w-full h-10 rounded-lg"
              >
                Cancelar
              </button>
              {isTransfiriendoTPV ? (
                <button disabled={true} className={`bg-blue-400 cursor-default w-full h-10 rounded-lg`}>
                  Transferir
                </button>
              ) : (
                <button
                  ...
                  onClick={Transferir}
                  className={`${
                    !empleadoSeleccionado ? "bg-blue-400 cursor-default" : "bg-blue-500 hover:bg-blue-600"
