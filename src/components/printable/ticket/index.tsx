      ...
      <div className="flex flex-col gap-2 items-center bg-white rounded-2xl w-full h-auto text-xs" ref={ref}>
                  ...
                  if (prod.dto) {
                    return (
                      <GenerarFilaProducto
                        ...
                        precio={Number(Number(prod.precioVenta) * (1 - Number(prod.dto) / 100))}
                      />
                    );
                  } else {
                    return (
        ...
        <div className="flex flex-col justify-evenly w-full h-auto items-center">
          ...
          {props.pagoCliente.pagoEnEfectivo > 0 && (
            <div>Pagado con efectivo: {props.pagoCliente.pagoEnEfectivo.toFixed(2)}€</div>
          )}
          {props.pagoCliente.pagoEnTarjeta > 0 && (
            <div>Pagado con tarjeta: {props.pagoCliente.pagoEnTarjeta.toFixed(2)}€</div>
          )}
        ...
        {props.pagoCliente.cliente.nif !== "General" && (
          <div className="flex flex-col text-center">
            ...
            {props.pagoCliente.cliente.nif && props.pagoCliente.cliente.nif !== "General" && (
              <div>CIF: {props.pagoCliente.cliente.nif} </div>
            )}
            {props.pagoCliente.cliente.calle && props.pagoCliente.cliente.calle !== "General" && (
              <div>{props.pagoCliente.cliente.calle} </div>
            )}
            {props.pagoCliente.cliente.cp && props.pagoCliente.cliente.cp !== "General" && (
              <div>CP: {props.pagoCliente.cliente.cp} </div>
            )}
          </div>
        )}
        <div className="flex flex-col pb-2">
          <span className="text-center">Le ha atendido {props.venta.vendidoPor.nombre}</span>
...
const GenerarFilaProducto = (props: { numFila: number; nombreProducto: string; cantidad: number; precio: number }) => {
  return (
    <div className="flex w-full">
      ...
      <div className="w-1/5 text-center">{(props.precio * props.cantidad).toFixed(2)}€</div>
    </div>
  );
};
