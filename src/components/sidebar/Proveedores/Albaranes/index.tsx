...
const AlbaranesPage = () => {
    ...
    const Filtrar = async (f: string) => {
        if (f === "") { return; }
        if (!ValidateSearchString(f)) { notifyWarn("Albarán inválido"); return; }
                        ...
                        <div className="h-full w-full border-2 rounded-b overflow-y-scroll">
                            {
                                props.albaranes.length <= 0 ?
                                    ...
                                    :
                                    props.albaranes.slice((elementsPerPage * (currentPage - 1)), currentPage * elementsPerPage).map((p, index) => {
                                        return (
...
const FilaAlbaran = (props: { albaran: any, albaranes: any[], setAllAlbaranes: Function }) => {
    ...
    const SetCurrentAlbaran = (p: any | null) => {
        if (p === null) {
            const provs = props.albaranes.filter((p) => { return p._id !== albaran._id });
