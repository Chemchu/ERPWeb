...
const EmpleadosPage = () => {
  ...
  useEffect(() => {
    if (filtro === "") {
      setEmpleadosFiltrados(undefined);
    }
  }, [filtro]);
    ...
    return (
      <div className="flex flex-col h-full w-full bg-white rounded-b-2xl rounded-r-2xl p-4 shadow-lg border-x">
        ...
        <div className="flex justify-between border-t border-x rounded-t-2xl px-5 py-2">
          ...
          <div className="text-left font-semibold w-1/3">Rol</div>
        </div>
        ...
        <AnimatePresence>{showModal && <AddEmpleado showModal={setModal} />}</AnimatePresence>
      </div>
    );
  }
      ...
      <AnimatePresence>
        {showModal && (
          <VerEmpleado
            ...
            setEmpleados={props.setEmpleados}
          />
        )}
      </AnimatePresence>
    </div>
