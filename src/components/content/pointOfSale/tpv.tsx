      ...
      startTransition(() => {
        ...
        if (stringValidated === "") productosFiltrados = props.productos;
          ...
          productosFiltrados = props.productos.filter((p: Producto) => {
            return (
              p.alta && (p.nombre.toUpperCase().includes(stringValidated.toUpperCase()) || p.ean === stringValidated)
            );
          });
        }
              ...
              <div className="flex gap-2 p-4">
                {arrayNum.map((n, i) => {
                  return (
                    <div
                      key={`SkeletonFav-${i}`}
                      className="animate-pulse h-10 w-16 md:w-32 lg:w-48 border-2 rounded-md mx-auto bg-gray-300"
                    />
                  );
                })}
              </div>
            ...
            <div className="px-2 text-lg h-16">
              <div className="flex w-full gap-2 flex-row bg-white rounded-3xl shadow transition-shadow focus:outline-none p-2">
                ...
                <input
                  ...
                  ref={inputRef}
                  onChange={(e) => {
                    setDirtyInput(e.target.value);
                  }}
                  value={dirtyInput}
                />
                <button
                  className="flex p-2 items-center rounded-full text-gray-400 hover:text-red-500"
                  onClick={() => {
                    LimpiarFiltro();
                  }}
                >
                ...
                <div className="px-4">
                  <div className="flex w-full max-h-20 py-2 gap-2 overflow-y-hidden overflow-x-scroll justify-start text-center">
                    <motion.button
                      ...
                      key={"Todos"}
                      id={"Todos"}
                      className="flex bg-blue-400 hover:bg-blue-500 text-white rounded-xl w-auto max-w-lg py-1 px-2"
                      onClick={() => {
                        setProductosFiltrados(props.productos);
                      }}
                    >
                    ...
                    {Familias.map((f) => {
                      return (
                        <motion.button
                          ...
                          className="flex bg-blue-400 hover:bg-blue-500 text-white rounded-xl w-auto max-w-lg py-1 px-2"
                          onClick={(e) => {
                            setProductosFiltrados(props.productos.filter((p) => p.familia === e.currentTarget.id));
                          }}
                        >
                          <span className="self-center w-full p-1 truncate">{f}</span>
                        </motion.button>
                      );
                    })}
                  </div>
          ...
          {props.productosFiltrados.slice(0, maxItems).map((prod: Producto) => {
            return (
              <button
                ...
                id={prod._id}
                onClick={() => {
                  AddProductoToCarrito(prod, ProductosEnCarrito, SetProductosEnCarrito);
                }}
              >
