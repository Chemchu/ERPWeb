...
const SidebarDerecho = React.memo(
  (props: {
    ...
    const [PrecioTotalFinal, setPrecioTotalFinal] = useState<number>(
      AplicarDescuentos(ProductosEnCarrito, Number(DtoEfectivo), Number(DtoPorcentaje))
    );
    ...
    const { EmpleadoUsingTPVState, AbrirCajaState, CerrarCajaState, TransferirTPVState } = useTpvStateContext();
    ...
    const SetPropiedadProd = useCallback((idProd: string, cantidad: string, dto: string, precioVenta?: string) => {
      if (!IsPositiveIntegerNumber(String(cantidad))) {
        return;
      }
      if (!IsPositiveFloatingNumber(dto)) {
        return;
      }
      ...
      return (
        <div className="bg-white grid grid-rows-2 grid-cols-1 h-full rounded-l-3xl shadow">
          <div className="flex flex-col gap-2 justify-items-center justify-self-center opacity-25 self-end">
            ...
            <p className="self-center">Carrito vacío</p>
          </div>
            ...
            {EmpleadoUsingTPVState.isEmpleadoUsingTPV ? (
              <div className="flex flex-col w-full items-center gap-2 pb-6">
                <button
                  ...
                  onClick={() => {
                    CerrarCajaState.setShowCerrarCajaModal(true);
                  }}
                >
                  Cerrar caja
                ...
                <button
                  className="flex gap-2 hover:text-blue-500"
                  onClick={() => {
                    TransferirTPVState.setShowTransferirTPVModal(true);
                  }}
                >
                  Transferir caja
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
              ...
              <button
                className="flex gap-2 hover:text-blue-500 w-full h-20 items-center justify-center"
                onClick={() => {
                  AbrirCajaState.setShowAbrirCajaModal(true);
                }}
              >
                Abrir caja
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
    ...
    return (
      <div className="flex flex-col justify-start items-center rounded-3xl bg-white shadow h-full max-h-full">
        <div className="flex justify-around items-center w-full h-20">
          ...
          <div
            className="flex justify-center text-blue-gray-300 hover:text-orange-500 focus:outline-none cursor-pointer w-10"
            onClick={() => {
              setSaveCompra(true);
            }}
          >
        ...
        <div className="h-1/3 xl:h-1/4 p-3 w-full">
          <div className="relative">
                  ...
                  <div className="flex self-center gap-2">
                    ...
                    <div>
                      <input
                        type="text"
                        inputMode="numeric"
                        className="text-xs text-center rounded-lg w-1/2 h-6 shadow"
                        name="DtoEfectivo"
                        value={DtoEfectivo}
            ...
            <div className="w-full flex justify-between text-left text-lg font-semibold">
              <div
                ...
                onClick={() => setDescuentoPopup(!DescuentoOpen)}
              >
                Descuento
              </div>
              {PrecioTotal === PrecioTotalFinal && (
                <div
                  className="hover:text-blue-500 underline cursor-pointer"
                  onClick={() => setOfertaPopup(!OfertaOpen)}
                >
                  Ofertas
                </div>
              )}
            ...
            {PrecioTotal !== PrecioTotalFinal ? (
              <div className="flex gap-2 justify-end ml-auto">
                <div className="text-right w-full text-red-500 line-through">
                  {ProductosEnCarrito.length <= 0 ? 0.0 : PrecioTotal.toFixed(2)} €
                </div>
                <div className="text-right w-full">
                  {ProductosEnCarrito.length <= 0 ? 0.0 : PrecioTotalFinal.toFixed(2)} €
                </div>
              </div>
            ) : (
              <div className="text-right w-full">{ProductosEnCarrito.length <= 0 ? 0.0 : PrecioTotal.toFixed(2)} €</div>
            )}
          </div>
          {!isNaN(PrecioTotal) && (
            <div className="flex flex-col w-full gap-2">
              {PrecioTotal > 0 && !isButtonDisabled && isVentaValida ? (
                <>
                  <motion.button
                    disabled={isButtonDisabled}
                    whileTap={{ scale: 0.9 }}
                    className={`bg-blue-500 h-10 xl:h-12 shadow rounded-lg hover:shadow-lg hover:bg-blue-600 text-white focus:outline-none`}
                    onClick={(e) => {
                      setPagarModal(true);
                    }}
                  >
                  ...
                  <motion.button
                    ...
                    className="bg-blue-500 h-10 xl:h-12 shadow rounded-lg hover:shadow-lg hover:bg-blue-600 text-white focus:outline-none"
                    onClick={async () => {
                      await Vender(PagoRapido, ProductosEnCarrito, Empleado, Clientes);
                    }}
                  >
                    COBRO RAPIDO
                  </motion.button>
                ...
                <>
                  <motion.button
                    className={`bg-blue-400 h-10 xl:h-12 shadow rounded-lg text-white focus:outline-none`}
          ...
          <AnimatePresence>
            ...
            {showModalSaveCompra && (
              <GuardarCompra
                compraActual={ProductosEnCarrito}
                setCompraActual={SetProductosEnCarrito}
                setModal={setSaveCompra}
              />
            )}
            {OfertaOpen && <Ofertar setModal={setOfertaPopup} productos={props.productos} />}
          </AnimatePresence>
        </div>
        {VentaResultante && PagoRapido && (
...
const GenerarProductList = React.memo(
  (props: { productosEnCarrito: ProductoVendido[]; setPropiedadProducto: Function }) => {
