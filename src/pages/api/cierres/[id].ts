...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    switch (method) {
      case "GET":
        if (Object.keys(query).length > 0) {
          return await GetCierresFromQuery(query, res);
        } else {
          return res.status(300).json({
            message: `Las consulta no puede estar vacía`,
            successful: false,
          });
        }
      case "POST":
    ...
    return res.status(300).json({
      message: `Solo se acepta metodo GET, POST y DELETE`,
      successful: false,
    });
  } catch (err) {
    console.log(err);
