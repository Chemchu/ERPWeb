    ...
    switch (method) {
      case "GET":
        if (Object.keys(query).length > 0) {
          return await GetEmpleadosFromQuery(query, res);
        } else {
          return await GetEmpleadoFromId(req, res);
        }
...
const GetEmpleadosFromQuery = async (userQuery: queryString.ParsedQuery<string>, res: NextApiResponse) => {
  if (!userQuery.query && !userQuery.isLibre) {
...
const DeleteEmpleado = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const data = JSON.parse(apiResponse.data);
  return res.status(data.deleteEmpleado.successful ? 200 : 300).json({
    message: data.deleteEmpleado.message,
    successful: data.deleteEmpleado.successful,
  });
};
