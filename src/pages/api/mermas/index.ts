...
const AddMerma = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const data = JSON.parse(apiResponse.data);
    return res.status(data.addMerma.successful ? 200 : 300).json({
      message: data.addMerma.message,
      successful: data.addMerma.successful || false,
    });
  } catch (err) {
    console.log(err);
    return res.status(500).json({
      message: `Error al intentar la merma: ${err}`,
      successful: false,
    });
  }
};
