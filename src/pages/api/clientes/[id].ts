...
import {
  ADD_CLIENTES_FILE as ADD_CLIENTS_FILE,
  DELETE_CLIENT,
  QUERY_CLIENT,
  QUERY_CLIENTS,
  UPDATE_CLIENT,
    ...
    switch (req.method) {
      ...
      case "GET":
        if (Object.keys(query).length > 0) {
          return await GetClientesFromQuery(query, res);
        }
...
const GetClienteFromId = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (!reqBody.find) {
    return res.status(300).json({ message: "La búsqueda no puede estar vacía", successful: false });
  }
