    ...
    switch (req.method) {
      case "GET":
        if (query.isTpvFree !== undefined) {
          return await GetTpvsByDisponibilidad(req, res);
        } else {
          return await GetTpvById(req, res);
        }

      case "PUT":
        if (query.transferirTpv !== undefined && query.transferirTpv) {
          return await TransferirTPV(req, res);
...
const OcuparTpvById = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    return res.status(300).json({
      message: `Fallo al buscar la TPV`,
      successful: data.ocupyTPV.successful,
    });
  } catch (err) {
...
const TransferirTPV = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    return res.json({
      message: data.transferirTpv.message,
      successful: data.transferirTpv.successful,
    });
  } catch (err) {
