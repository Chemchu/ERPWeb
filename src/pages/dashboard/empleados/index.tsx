...
import EmpleadosPage from "../../../components/content/Empleados";
...
const renderTabPages = (currentElemento: string) => {
  switch (currentElemento) {
    case "Empleados":
      return (
        <div key={"Empleados"} className={classNames("h-full w-full", "focus:outline-none ring-white ring-opacity-60")}>
          <EmpleadosPage />
        </div>
      );
    ...
    case "Turnos":
      return (
        <div key={"Turnos"} className={classNames("h-full w-full", "focus:outline-none ring-white ring-opacity-60")}>
          <EnDesarrolloPage />
        </div>
      );
...
const Empleados = (props: { EmpleadoSesion: SesionEmpleado }) => {
  ...
  const [currentElemento, setElemento] = useState<string>("Empleados");
  ...
  return (
    <main className="w-full h-full">
      ...
      <Tab.Group as="div" className="hidden sm:flex sm:flex-col w-full h-full">
        <Tab.List className="flex sm:gap-1 grow-0 h-10 sm:pr-10">
          <Tab
            key={"Empleados"}
          ...
          >
            <span className="text-xl">Turnos</span>
          </Tab>
        </Tab.List>
