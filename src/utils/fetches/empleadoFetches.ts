...
export const FetchEmpleado = async (_id: string): Promise<Empleado | undefined> => {
  if (!_id) {
    throw "ID del empleado no puede ser undefined";
  }
  ...
  try {
    ...
    if (!fetchRes.ok) {
      notifyError("Error al buscar al empleado");
      return undefined;
    }
...
export const FetchCurrentUser = async (): Promise<Empleado> => {
  try {
    ...
    if (!fetchRes.ok) {
      notifyError(empleadoJson.message);
      return {} as Empleado;
    }
...
export const FetchEmpleadosByQuery = async (userQuery: string): Promise<Empleado[]> => {
  try {
    ...
    if (!pResponse.ok) {
      notifyError("Error al buscar los empleados");
      return [];
    }
