...
export const FetchTPV = async (TPVId: string, abortController: AbortController): Promise<ITPV | undefined> => {
  ...
  try {
    ...
    if (!fetchTPV.ok) {
      notifyError("Error al buscar la TPV");
      return undefined;
    }
...
export const TransferirTPV = async (
  tpvId: string,
  empDestinoId: string
...
export const FetchTPVsByDisponibilidad = async (
  isTpvFree: boolean,
  abortController: AbortController
): Promise<ITPV[] | undefined> => {
  try {
